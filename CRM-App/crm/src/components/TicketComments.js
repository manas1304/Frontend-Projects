...
export default function TicketComments({ ticketId }) {
  ...
  useEffect(() => {
    const socket = io("https://backend-projects-5g25.onrender.com"); // Using backend Render backend URL

    // Join the room for the specific ticket
    socket.emit("joinTicket", ticketId);

    // Listen to new comments on the server
    socket.on("newComment", (incomingComment) => {
      // Only add a comment if it's not already on the list(prevents duplicates from the sender)
      setComments((prev) => {
        const exists = prev.find((c) => c._id === incomingComment._id);
        return exists ? prev : [...prev, incomingComment];
      });
    });
